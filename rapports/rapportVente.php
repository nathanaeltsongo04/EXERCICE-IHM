...
if ($_GET['datedebut'] && !empty($_GET['datedebut']) && $_GET['datefin'] && !empty($_GET['datefin'])) {
  $con = new db_connection();
  $connect = $con->openconnection();
  $datedebut = strip_tags($_GET['datedebut']);
  ...
  $data->bindValue(':datedebut', $datedebut, PDO::PARAM_STR);
...
<body>
  <div class="invoice">
    <div class="header">
      <h1>Rapport de vente du '.$datedebut.' au '.$datefin.'</h1>
    </div>
    <table>
      <tr>
            <th>Client</th>
          ...
          $html .= '<tr>
                <td colspan="6" style="text-align:right; font-weight: bold;font-size:18px;">'.$out.'</td>
            </tr>';
